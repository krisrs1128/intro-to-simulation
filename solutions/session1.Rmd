...
experiment. This experiment (tk fill in a bit about what it was about, Lahti2014).
...
trends? This is the basis for defining synthetic negative controls.

```{r}
sim <- sim |>
  mutate(1:6, link = ~ ns(time, df = 7)) |>
  estimate(nu = 0.01, mstop = 1000)
...
**Exercise**: How well does this simulator approximate the original template?
*The block below
draws a hypothetical experiment from the original data. Try comparing the
original `atlas` with the simulated `atlas_` experiments.

_Hint: You can use `pivot_experiment` to convert a `SummarizedExperiment` object into an ordinary data.frame._
