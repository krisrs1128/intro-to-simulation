...
update_simulator <- function(simulator, n_nonnull, signal_order) {
  ...
  simulator |>
    scDesigner::mutate(any_of(nonnull), link = ~outcome2) |>
    estimate() |>
    suppressMessages()
}

sample_n <- function(simulator, n_sim = 100) {
  n_original <- ncol(simulator@template)
  rep_ix <- rep(seq_len(n_original), each = ceiling(n_sim / n_original))
  new_data <- colData(simulator@template)[rep_ix[1:n_sim], ]
  new_data$outcome2 <- c(rep("healthy", n_sim / 2), rep("T1D", n_sim / 2))

  sample(simulator, new_data = new_data)
}
```
